...
pool = d2l.QuestionPool( "three_input_decoder_in_out", "pool.csv" )
...
for num_in in range(0, max_val):
    ...
    dec = decoderDraw(size, s="abc", out_ind=num_in)
    question_text = "<p>What is the binary input, abc, for the following : " \
        f"decoder? {dec}</p>"
    question = d2l.MSQuestion(question_text)
    # question.add_answer(f"(?i)^\s*(0b)?0*{bin_in}\s*$", is_regex = True)
    for i in range(max_val):
        option = bin(i)[2:].zfill(size)
        question.add_answer(f"{option}", is_correct = i == num_in  )
    pool.add_question(question)
