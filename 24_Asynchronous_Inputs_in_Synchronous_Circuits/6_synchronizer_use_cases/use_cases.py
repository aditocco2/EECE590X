...
correct = [
    ...
    ("A signal coming from a different system with the same clock frequency", True), # debatable
...
wrong = [
    ("An input that is running on the same clock as the rest of the system", False),
    ("An internal signal going from one register to another in the same system", False),
    ("A signal going directly to the LEDs on the FPGA board", False),
    ("A signal going into a purely combinational circuit", False) # also debatable
]
