...
image_name = "glitchy_circuit.png"
# I just added the labels manually for this one ¯\_(ツ)_/¯

pool = d2l.QuestionPool("Compare ideal and non-ideal outputs, MS", "pool.csv")
...
question.add_answer(f"<p>Whenever <b>more than 4 ns</b> has elapsed " \
...
question.add_answer(f"This circuit can suffer glitches, which can " \
                    f"persist for <b>longer than 4 ns</b> per single glitch.", 
                    is_correct= False)

question.add_answer(f"This circuit can suffer glitches, which " \
                    f"persist for <b><i>no</i> longer than 4 ns</b> per single glitch.",
                    is_correct = True)
