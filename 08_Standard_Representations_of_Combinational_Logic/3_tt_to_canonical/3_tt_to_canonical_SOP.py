...
for i in range(number_questions):
    ...
    question_text= f"<p> Which of the following is the canonical SOP " \
            f"expression for the below truth table? {tt}</p>"
    question = d2l.MCQuestion( question_text )
    
    question.add_answer(f"{ans}", 100)
    question.add_answer(f"{wrong1}", 0)
